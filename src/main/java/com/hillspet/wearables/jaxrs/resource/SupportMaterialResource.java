 ...
 */
@Path("/supportDocs")
@Api(value = "RESTful service that performs support document related operations", tags = {
...
public interface SupportMaterialResource {
	...
	@GET
	@Path("/getSupportDocs")
	@ApiOperation(value = "Get All support documents", notes = "Get All support documents")
	@ApiResponses(value = {
	...
	@ApiOperation(value = "Get Campaign List", notes = "Gets the Campaign List", hidden = true)
	@ApiResponses(value = {
	...
	@ApiResponses(value = {
			...
			@ApiResponse(code = HttpStatus.SC_FORBIDDEN, message = "Forbidden"),
			@ApiResponse(code = HttpStatus.SC_INTERNAL_SERVER_ERROR, message = "Runtime Error or Internal Server Error") })
	public Response getMaterialCategoryList(@QueryParam("categoryId") int categoryId,
			@HeaderParam("ClientToken") String token);

	@GET
	@Path("/getDeviceSupportDocs/{deviceType}/{deviceModel}")
	@ApiOperation(value = "Get Sensor support documents by deviceType and device model", notes = "Get Sensor support documents by deviceType and device model", hidden = true)
	@ApiResponses(value = {
			@ApiResponse(code = HttpStatus.SC_OK, message = "Successful Response", response = SupportMaterialsResponse.class),
			@ApiResponse(code = HttpStatus.SC_BAD_REQUEST, message = "Bad Request"),
			@ApiResponse(code = HttpStatus.SC_NOT_FOUND, message = "Not Found"),
	...
	@ApiOperation(value = "Get App support documents by sub categroy type", notes = "Get App support documents by sub categroy type", hidden = true)
	@ApiResponses(value = {
			@ApiResponse(code = HttpStatus.SC_OK, message = "Successful Response", response = SupportMaterialsResponse.class),
			@ApiResponse(code = HttpStatus.SC_BAD_REQUEST, message = "Bad Request"),
			@ApiResponse(code = HttpStatus.SC_NOT_FOUND, message = "Not Found"),
